...
func (s *Service) cleanupPendingClosures(ctx context.Context, duration time.Duration) (int, error) {
	...
	// One cutoff for both the multipart selection and the closure deletion,
	// taken from the database clock. Recomputing now() in the delete would
	// reap closures that aged past the cutoff in between without aborting
	// their multipart uploads, leaving them untracked in S3.
	dbNow, err := queries.Now(ctx)
	if err != nil {
		return 0, fmt.Errorf("read database clock: %w", err)
	}

	cutoff := pgtype.Timestamp{Time: dbNow.Time.Add(-duration), Valid: true}
